...
class PersistentNestedStateSpec extends
  Spec with 
  ShouldMatchers with 
  BeforeAndAfterAll {  
  ...
  override def beforeAll {
    Config.config
    conf.configure(
        new RestartStrategy(new AllForOne, 3, 5000, new Array[Class](classOf[Exception])),
        new Array[Component](
          new Component(classOf[PersistentStateful],
              new LifeCycle(new Permanent),
              10000),
          new Component(classOf[PersistentStatefulNested],
              new LifeCycle(new Permanent),
              10000),
          new Component(classOf[PersistentFailer],
              new LifeCycle(new Permanent),
              10000)
        )).supervise
  }
  ...
  describe("Transactional nested persistent Active Object") {

    it("map should not rollback state for stateful server in case of success") {
      val stateful = conf.getInstance(classOf[PersistentStateful])
      stateful.setMapState("testShouldNotRollbackStateForStatefulServerInCaseOfSuccess", "init") // set init state
      Thread.sleep(100)
      val nested = conf.getInstance(classOf[PersistentStatefulNested])
    ...
    it("map should rollback state for stateful server in case of failure") {
      ...
      Thread.sleep(100)
      val failer = conf.getInstance(classOf[PersistentFailer])
      try {
