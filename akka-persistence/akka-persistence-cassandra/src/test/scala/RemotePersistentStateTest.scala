...
class RemotePersistentStateSpec extends
  Spec with 
  ShouldMatchers with 
  BeforeAndAfterAll {  
  ...
  describe("Remote transactional persistent Active Object ") {

    it("map should not rollback state for stateful server in case of success") {
      val stateful =  ActiveObject.newRemoteInstance(classOf[PersistentStateful], 1000, "localhost", 9999)
    ...
    it("map should rollback state for stateful server in case of failure") {
      ...
      val failer =ActiveObject.newRemoteInstance(classOf[PersistentFailer], 1000, "localhost", 9999) //conf.getInstance(classOf[PersistentFailer])
      try {
